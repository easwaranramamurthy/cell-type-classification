...
class SelfAttention(nn.Module):
    def __init__(self, num_tokens = 2048, d_model=512, d_q_k_v=128, num_heads=6):
        ...
        self.value = nn.Linear(in_features=d_model, out_featuers=d_q_k_v*num_heads)
    ...
    def forward(self, embedding):
        ...
        token_weights_pre_softmax = torch.einsum('bthd, bthd->btth', queries, keys)
        token_weights_pre_softmax_scaled = torch.div(token_weights_pre_softmax, torch.sqrt(self.d_model))
